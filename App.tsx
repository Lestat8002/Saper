...
import { GameStatus, Difficulty } from './utils/types';
import { DIFFICULTY_SETTINGS } from './components/constants';
...
const App: React.FC = () => {
  ...
  const resetGame = useCallback((newDifficulty: Difficulty) => {
    setBackgroundImage(currentBg => {
      if (IMAGES.length <= 1) return currentBg;
      let newBg;
      do {
  ...
  const handleCellContextMenu = useCallback((e: React.MouseEvent<HTMLElement>, row: number, col: number) => {
    e.preventDefault();
    handleToggleMark(row, col);
  }, [handleToggleMark]);
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-4 font-sans">
        ...
        />
        <div className="relative my-4 flex flex-col items-center space-y-4">
            <Board
                board={board}
